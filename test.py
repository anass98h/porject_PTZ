...
class Viewport:
    ...
    def adjust_view_zoom(self, mouse_pos, direction):
        ...
        self.x_offset = (
            (self.x_offset + (mouse_x - self.view_size[0] / 2) / old_zoom_level)
            * old_zoom_level
            / self.zoom_level
        )
        self.y_offset = (
            (self.y_offset + (mouse_y - self.view_size[1] / 2) / old_zoom_level)
            * old_zoom_level
            / self.zoom_level
        )
        ...
        self.y_offset = max(
            0, min(self.panorama.get_height() - zoomed_height, self.y_offset)
        )  # Clamp vertically

    def update_position(self, keys, key_map):
        step_size = 10  # pixels per frame
        if keys[key_map["left"]]:
            self.x_offset -= step_size / self.zoom_level
        if keys[key_map["right"]]:
            self.x_offset += step_size / self.zoom_level
        if keys[key_map["up"]]:
            self.y_offset -= step_size / self.zoom_level
    ...
    def render(self, target_surface, position=(0, 0)):
        if self.is_static:
            ...
            if self.x_offset + visible_width > self.panorama.get_width():
                ...
                part_surface1 = self.panorama.subsurface(
                    (int(self.x_offset), int(self.y_offset), part_width, visible_height)
                )
                part_surface1_scaled = pygame.transform.scale(
                    part_surface1,
                    (
                        int(part_width * self.view_size[0] / visible_width),
                        self.view_size[1],
                    ),
                )
                target_surface.blit(part_surface1_scaled, position)

                part_surface2 = self.panorama.subsurface(
                    (0, int(self.y_offset), part_remainder, visible_height)
                )
                part_surface2_scaled = pygame.transform.scale(
                    part_surface2,
                    (
                        int(part_remainder * self.view_size[0] / visible_width),
                        self.view_size[1],
                    ),
                )
                target_surface.blit(
                    part_surface2_scaled,
                    (
                        position[0]
                        + int(part_width * self.view_size[0] / visible_width),
                        position[1],
                ...
                visible_part_scaled = pygame.transform.scale(
                    visible_part, self.view_size
                )
...
class ZoomSlider:
    ...
    def draw(self, surface):
        ...
        pygame.draw.rect(
            surface, (200, 200, 200), pygame.Rect(self.position, self.size)
        )
        # Calculate the slider handle position
        handle_height = 20
        handle_y = self.position[1] + (self.size[1] - handle_height) * (
        ...
        pygame.draw.rect(
            surface,
            (100, 100, 250),
            (self.position[0], handle_y, self.size[0], handle_height),
        )
    ...
    def set_zoom(self, new_zoom):
        if self.min_zoom <= new_zoom <= self.max_zoom:
            for camera in [camera_1, camera_2]:
                ...
                center_x = (
                    camera.x_offset + (camera.view_size[0] * camera.zoom_level) / 2
                )
                center_y = (
                    camera.y_offset + (camera.view_size[1] * camera.zoom_level) / 2
                )
                ...
                camera.x_offset = max(
                    0,
                    min(
                        camera.panorama.get_width() - camera.view_size[0] * new_zoom,
                        center_x - (camera.view_size[0] * new_zoom) / 2,
                    ),
                )
                camera.y_offset = max(
                    0,
                    min(
                        camera.panorama.get_height() - camera.view_size[1] * new_zoom,
                        center_y - (camera.view_size[1] * new_zoom) / 2,
                    ),
                )

            # Ensure camera 2 remains aligned to the right of camera 1 for panoramic alignment
            camera_2.x_offset = (
                camera_1.x_offset + get_camera_field_of_view(camera_1)
            ) % camera_2.panorama.get_width()
...
def handle_mouse_click(mouse_x, mouse_y):
    ...
    camera_1.x_offset = (
        max(
            0,
            min(
                camera_1.panorama.get_width()
                - camera_1.view_size[0] * camera_1.zoom_level,
                camera_1.x_offset,
            ),
        )
        % camera_1.panorama.get_width()
    )
    camera_1.y_offset = max(
        0,
        min(
            camera_1.panorama.get_height()
            - camera_1.view_size[1] * camera_1.zoom_level,
            camera_1.y_offset,
        ),
    )
    ...
    camera_2.x_offset = (
        camera_1.x_offset + camera_1.view_size[0] * camera_1.zoom_level
    ) % camera_2.panorama.get_width()
    camera_2.y_offset = camera_1.y_offset
    ...
    if (
        camera_2.x_offset + camera_2.view_size[0] * camera_2.zoom_level
        > camera_2.panorama.get_width()
    ):
...
def handle_events():
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if (
                event.button == 1
            ):  # Left click to center cameras and adjust for panoramic view
                handle_mouse_click(mouse_x, mouse_y)


def update_views():
    keys = pygame.key.get_pressed()
    camera_1.update_position(
        keys,
        {"left": pygame.K_a, "right": pygame.K_d, "up": pygame.K_w, "down": pygame.K_s},
    )
    camera_2.update_position(
        keys,
        {"left": pygame.K_a, "right": pygame.K_d, "up": pygame.K_w, "down": pygame.K_s},
    )
